...
something_to_dramatize.set_text("Солдатові дано місію, донести прапор на землі майбутнього, і забравши звідти новітню зброю, повернутись на позиції.")
...
game_artists.set_text("Художники: Юник Вадим, DALL-E, Юник Віктор")
testers = Label(25, 350, 100, 50)
testers.set_text("Тестери: Юник Вадим")

#об'єкти для гри
finish = Label(100, width // 6, 100,50)
finish.set_text("Дякую за прапор! Ось візьми, ця зброя для тебе. Тепер повернись на позиції, але обережно, земля замінована!")
finish2 = Label(200, width // 5, 100, 50)
finish2.set_text("(Спустіться на най нижчу платформу, але не на землю)")
WIN = Label(width // 2 - 100, width // 4, 100, 50)
WIN.set_text("Місія виконана!")
lose = Label(width // 2 - 100 ,width // 4, 100, 50)
lose.set_text("Місія не виконана!")
...
player_image = pygame.transform.scale(player_image, (50, 65))
flag_image = pygame.image.load("image/flag.png")
flag_image = pygame.transform.scale(flag_image, (50, 50))
weapon_image = pygame.image.load("image/future_weapon.png")
...
class Player:
    ...
    def update(self):
        ...
        # if self.rect.colliderect(finish_platform.rect) and self.has_flag:
        #     self.has_flag = False
        #     self.has_weapon = True
        #     self.can_win = True
            
        # if not self.has_flag:
        #     window.blit(flag_image, (finish_platform.rect.x + 200, finish_platform.rect.y - camera_y - 45))
        # if not self.has_weapon:
        #     window.blit(weapon_image, (finish_platform.rect.x + 200, finish_platform.rect.y - camera_y - 45))

    def draw(self):
        window.blit(player_image, (self.rect.x, self.rect.y - camera_y))
        if self.has_flag:
            window.blit(flag_image, (self.rect.x - 5, self.rect.y - camera_y - 25))
        if self.has_weapon:
            window.blit(weapon_image, (self.rect.x + 5, self.rect.y - camera_y + 10))
...
player = Player(width // 2 - 25, height - 60, 25, 65, 5)
...
platform9 = Platform(window,-350,-450,500, 20, 0, 1, dark_green,pygame.image.load("image/platform 500x20.png"))
...
# base_platform = Platform(window,0, height - 40, width, 2000, 0, 0, green)
# finish_platform = Platform(window,0, -7550, width - 100, 30, 0, 0, green)

platforms = [platform1, platform2, platform3, platform4,platform5, platform6, platform7, platform8,platform9,platform8_9,platform10,platform11,platform12,platform13,platform14,platform15,platform16,platform17,platform18,platform14_2,platform15_2,platform16_2,platform17_2,platform18_2,platform19,platform20,platform21,platform22,platform23,platform24,platform25,platform26,platform27,platform28,platform29,platform30,
platform31, platform32,platform33, platform34, platform35, platform36, platform37, platform38, platform39, platform40, platform41,platform42, platform43,platform44, platform45, platform46, platform46_47, platform47, platform48, platform49, platform50, platform51,platform52,platform53,platform54,platform55,platform56, platform57, platform58, platform59, platform60, platform61]
